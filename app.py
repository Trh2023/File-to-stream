...
async def catch_all_debug(client, message):
    print(
        f"🔵🔵🔵 UPDATE RECEIVED! chat_id={message.chat.id}, "
        f"from={message.from_user.id if message.from_user else 'N/A'}, text={message.text}"
    )


@bot.on_message(filters.command("start") & filters.private)
async def start_command(client: Client, message: Message):
    print(f"🟢 DEBUG: /start ended payam az user {message.from_user.id} rasid")
    try:
...
async def simple_gatekeeper(c: Client, m_update: ChatMemberUpdated):
    try:
        if (
            m_update.new_chat_member
            and m_update.new_chat_member.status == enums.ChatMemberStatus.MEMBER
        ):
...
async def get_file_details_api(request: Request, unique_id: str):
    ...
    response_data = {
        ...
        "mx_player_link": (
            f"intent:{Config.BASE_URL}/dl/{message_id}/{safe_file_name}"
            f"#Intent;action=android.intent.action.VIEW;type={mime_type};end"
        ),
        "vlc_player_link": (
            f"intent:{Config.BASE_URL}/dl/{message_id}/{safe_file_name}"
            f"#Intent;action=android.intent.action.VIEW;type={mime_type};"
        ...
        if rh:
            ...
            try:
                ...
                if not rps[0]:
                    ...
                    suffix_len = int(rps[1])
                    if suffix_len <= 0:
                        raise ValueError
                    fb = max(0, fsize - suffix_len)
                    ub = fsize - 1
                else:
                    fb = int(rps[0])
